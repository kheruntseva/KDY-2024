		...
		if (!isComment)
		{
			...
			if (isStopSymbol(symb))
			{
				...
				*/
				...
				/*
				else if (isFunction(words[i])) // Если это функция
				{
					std::string funcName = words[i];
					if (funcName == "division")
					{
						// Проверяем, есть ли открывающая скобка
						int argIndex = i + 1;
						if (argIndex < words.size() && words[argIndex] == "(")
						{
							// Считываем аргументы
							...
							while (argIndex < words.size() && openParen > 0)
							{
								if (words[argIndex] == "(")
									openParen++;
								else if (words[argIndex] == ")")
									openParen--;

								if (openParen > 0)
									args.push_back(words[argIndex]);

								argIndex++;
							}

							// Проверяем, что у функции два аргумента
							if (args.size() == 2)
							{
								std::string firstArg = args[0];
								std::string secondArg = args[1];

								// Проверяем, является ли второй аргумент 0
								if (isLiteral(secondArg) && secondArg == "0")
								{
									throw ERROR_THROW_IN(655, line, -1); // Код ошибки 616: деление на ноль
								}
							}
							else
							{
								throw ERROR_THROW_IN(655, line, -1); // Код ошибки 617: неправильное количество аргументов
							}
						}
					}
				}
...
bool isBool(string& fullWord) {
	if (fullWord == "true" || fullWord == "false") {
		return true;
	}
	return false;
}

bool istrue(string& fullWord)
	{
if (fullWord == "true") { return true; }
else if (fullWord == "false") { return false; }
else return false;
	}
	...
	for (int i = 0; i < words.size(); i++)
	{
			...
			throw ERROR_THROW_IN(609, line, -1);
				...
				else if (isLiteral(words[j]))
				{
					...
					ide->iddatatype = words[j][0] == '\'' ? IT::SYMB
						...
						: isBool(words[j]) ? IT::BOOLEAN :
						IT::NUM;
				}
				//вот тут закончила проверять
				else
				{
			...
			words[i + 1] == "=>")
		{
			*fullWord = words[i + 2];
			...
			else if (isLiteral(*fullWord))
			{
				// Обработка логических литералов
				if (*fullWord == "true" || *fullWord == "false")
				{
					words.insert(words.begin() + i + 2, "bool");
				}
				if (fullWord->c_str()[0] == '\'')
				{
				...
				else if(isUnsignedChar(*fullWord))
				{
			...
			else if (is_id_in_table(idtable, *fullWord, scope, fullWord))
			{
				...
				*tempId = IT::GetEntry(idtable, index);
				...
				else if (tempId->iddatatype == IT::BOOLEAN)
				{
					words.insert(words.begin() + i + 2, "bool");
				}
				else
		...
		/*
		else if (check(checker._true, word.c_str()))
		{
			lexe.idxTI = -1;
			lexe.lexema = 't';
			lexe.sn = line;
		...
		else if (check(checker._false, word.c_str()))
		{
			lexe.idxTI = -1;
			lexe.lexema = 't';
			lexe.sn = line;
		...
		*/
		...
		else if (check(checker._for, word.c_str()))
		{
			...
			else if ((index = IT::IsId(idtable, (char*)fullWord->c_str())) != TI_NULLIDX)
			{
				...
				*tempId = IT::GetEntry(idtable, index);
				lexe.view = (tempId->iddatatype == IT::NUM ? 'n' : tempId->iddatatype == IT::FLT ? 'f' : tempId->iddatatype == IT::BOOLEAN ? 'b' : 's');
		...
		else if (check(checker._switch, word.c_str()))
		{
			scope.push("$Switch" + to_string(countScopes++));
			lexe.lexema = SWITCH;
			lexe.sn = line;
			lexe.idxTI = -1;
		...
		{
		scope.push("$true" + to_string(countScopes++));
		lexe.lexema = TRUE;
		lexe.sn = line;
		lexe.idxTI = -1;
		...
		{
		scope.push("$false" + to_string(countScopes++));
		lexe.lexema = FALSE;
		lexe.sn = line;
		lexe.idxTI = -1;
		...
		*/
		...
		else if (check(checker._bool, word.c_str()))
		{
		lexe.idxTI = -1;
		lexe.lexema = 't';
		lexe.sn = line;
		...
		else if (check(checker._equalinit, word.c_str()))
		{
			...
			if (words[i - 1] == "num" || words[i - 1] == "symb" || words[i - 1] == "float" || words[i - 1] == "bool")
			{
				*fullWord = words[i - 3];
			}
		...
		else if (isId(word))
		{
			...
			if (
				words[i + 1] == "is" &&
				(words[i + 2] == "num" || words[i + 2] == "symb" || words[i + 2] == "float" || words[i + 2] == "bool" || words[i + 2] == "foo" ||
					(words[i + 2] == "ref" && (words[i + 3] == "num" || words[i + 3] == "symb" || words[i + 3] == "float" || words[i + 3] == "bool"))) &&
				words[i + 4] != "?" &&
				!in_scope
				)
			{
					...
					if (words[i + 3] == "(")
					{
						...
						while (words[j] != ")")
						{
							...
							if (words[j] == "is")
							{
								...
								if (
									j + itmp < words.size() &&
									(words[j + itmp] == "num" || words[j + itmp] == "symb" || words[j + itmp] == "float" || words[j + itmp] == "bool"))
								{
									if (words[j + itmp] == "num")
									{
										ide->params.push_back(itmp == 1 ? "unsigned char" : "unsigned char&");
									}
									...
									else if (words[j + itmp] == "bool")
									{
										ide->params.push_back(itmp == 1 ? "bool" : "bool&");
									}
								}
							}
						...
						if (
							words[j + 1] == "is" &&
							(words[j + 2] == "num" || words[j + 2] == "symb" || words[j + 2] == "action" || words[j + 2] == "float" || words[j + 2] == "bool")
							)
						{
							if (words[j + 2] == "action")
							{
							...
							else if (words[j + 2] == "num")
							{
								ide->iddatatype = IT::NUM;
							}
							else if (words[j + 2] == "symb")
							{
								ide->iddatatype = IT::SYMB;
							}
							else if (words[j + 2] == "bool")
							{
								ide->iddatatype = IT::BOOLEAN;
							}
						}
				...
				words[i - 1] == "(" &&
				words[i + 1] == "is" &&
				(words[i + 2] == "num" || words[i + 2] == "symb" || words[i + 2] == "float" || words[i + 2] == "bool") &&
				words[i + 3] == ")" &&
				words[i + 4] == "?" &&
				is_id_in_table(idtable, word, scope, fullWord)
				)
				...
				if (index != TI_NULLIDX)
				{
					*ide = idtable.table[index];
					...
					if (
						...
						(ide->iddatatype == IT::SYMB && words[i + 2] == "bool") ||
...
bool is16Num(string& word)
{
	FST reg16Num(
		...
		NODE(1, RELATION('$', 2)),
		NODE(32, RELATION('0', 3), RELATION('0', 2), RELATION('1', 3), RELATION('1', 2), RELATION('2', 3), RELATION('2', 2),
			RELATION('3', 3), RELATION('3', 2), RELATION('4', 3), RELATION('4', 2), RELATION('5', 3), RELATION('5', 2),
			RELATION('6', 3), RELATION('6', 2), RELATION('7', 3), RELATION('7', 2), RELATION('8', 3), RELATION('8', 2),
			RELATION('9', 3), RELATION('9', 2), RELATION('A', 3), RELATION('A', 2), RELATION('B', 3), RELATION('B', 2),
			RELATION('C', 3), RELATION('C', 2), RELATION('D', 3), RELATION('D', 2), RELATION('E', 3), RELATION('E', 2),
			RELATION('F', 3), RELATION('F', 2)),
		NODE()
	);
...
bool isNum(string& word)
{
	FST regNum(
		(char*)word.c_str(),
		3,
		NODE(24, RELATION('+', 1), RELATION('-', 1), RELATION('0', 1), RELATION('0', 2), RELATION('1', 1), RELATION('1', 2),
			RELATION('2', 1), RELATION('2', 2), RELATION('3', 1), RELATION('3', 2), RELATION('4', 1), RELATION('4', 2),
			RELATION('5', 1), RELATION('5', 2), RELATION('6', 1), RELATION('6', 2), RELATION('7', 1), RELATION('7', 2),
			RELATION('8', 1), RELATION('8', 2), RELATION('9', 1), RELATION('9', 2), RELATION('_', 1), RELATION('_', 2)),
		NODE(22, RELATION('_', 1), RELATION('_', 2), RELATION('0', 1), RELATION('0', 2), RELATION('1', 1), RELATION('1', 2),
			RELATION('2', 1), RELATION('2', 2), RELATION('3', 1), RELATION('3', 2), RELATION('4', 1), RELATION('4', 2),
			RELATION('5', 1), RELATION('5', 2), RELATION('6', 1), RELATION('6', 2), RELATION('7', 1), RELATION('7', 2),
...
bool isLiteral(string& word)
{
	...
	if (isOnly)
		return false;
	if (strcmp(word.c_str(), "false") == 0) { // Преобразуем std::string в const char*
		return true;
	}
...
bool isFloat(string& word)
{
	...
	FST fstFloat(
		(char*)word.c_str(),
		4,
		NODE(22, RELATION('_', 0), RELATION('.', 2), RELATION('0', 1), RELATION('0', 0), RELATION('1', 1), RELATION('1', 0),
			RELATION('2', 1), RELATION('2', 0), RELATION('3', 1), RELATION('3', 0), RELATION('4', 1), RELATION('4', 0),
			RELATION('5', 1), RELATION('5', 0), RELATION('6', 1), RELATION('6', 0), RELATION('7', 1), RELATION('7', 0),
			RELATION('8', 1), RELATION('8', 0), RELATION('9', 1), RELATION('9', 0)),
		NODE(1, RELATION('.', 2)),
		NODE(22, RELATION('_', 2), RELATION('_', 3), RELATION('0', 2), RELATION('0', 3), RELATION('1', 2), RELATION('1', 3),
			RELATION('2', 2), RELATION('2', 3), RELATION('3', 2), RELATION('3', 3), RELATION('4', 2), RELATION('4', 3),
			RELATION('5', 2), RELATION('5', 3), RELATION('6', 2), RELATION('6', 3), RELATION('7', 2), RELATION('7', 3),
			RELATION('8', 2), RELATION('8', 3), RELATION('9', 2), RELATION('9', 3), RELATION('_', 2), RELATION('_', 3)),
		NODE()
	);
...
bool isUnsignedChar(string& word)
{
	// Проверка, не начинается ли строка с недопустимого символа
	if (word.empty() || (!std::isdigit(word.front()) && word.front() != '+'))
		return false;

	char sign = '\0';
	bool hasSign = false;

	// Удаляем знак '+' в начале строки, если он присутствует
	if (word.front() == '+')
	{
		sign = word.front();
		hasSign = true;
	...
	// Проверка оставшейся строки с помощью FST
	FST fstUnsignedChar(
		(char*)word.c_str(),
		2,
		NODE(10, RELATION('0', 1), RELATION('1', 1), RELATION('2', 1), RELATION('3', 1), RELATION('4', 1),
			RELATION('5', 1), RELATION('6', 1), RELATION('7', 1), RELATION('8', 1), RELATION('9', 1)),
		NODE()
	);

	if (execute(fstUnsignedChar))
	{
		...
		try
		{
			...
			{
				if (hasSign)
					word = sign + word; // Восстановление знака
				return true;
			}
		}
		catch (const std::exception&)
		{
			// В случае ошибки преобразования возвращаем false
		}
	}

	// Восстанавливаем исходную строку, если был знак
	if (hasSign)
		word = sign + word;
